...
void cgl::CGLGameLoop::Run()
{
	...
	while (!m_quitting)
	{
		...
		if (timeAccount >= updateInterval)
		{
			...
			if (occluded)
			{
				HRESULT hResult = cgl::CGLManager::GetInstance()->GetDevice()->GetSwapChain()->Present(0, DXGI_PRESENT_TEST);
				switch ( hResult )
				{
		...
		if (drawn)
		{
			HRESULT hResult = CGLManager::GetInstance()->GetDevice()->GetSwapChain()->Present(0, 0);
			switch ( hResult )
			{
				// the device has been removed
				// recreation of d3d11 objects needed
