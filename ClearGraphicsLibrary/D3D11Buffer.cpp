...
void cgl::CD3D11Buffer::SetData( char* pData, size_t count )
{
	// delete old data
	ResetData();
...
void cgl::CD3D11Buffer::AddData( char* pData )
{
	int insertPos = m_data.size();
	m_data.resize(m_data.size() + m_stride);
	memcpy(&m_data.data()[insertPos], pData, m_stride);
}
void cgl::CD3D11Buffer::AddData( char* pData, size_t count )
{
	// copy data
