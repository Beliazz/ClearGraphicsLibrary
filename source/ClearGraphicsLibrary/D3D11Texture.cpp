...
cgl::PD3D11TextureFromMemory cgl::CD3D11TextureFromMemory::Create( void* pBuffer, size_t size , D3DX11_IMAGE_LOAD_INFO loadInfo )
{
	return create<CD3D11TextureFromMemory>(new CD3D11TextureFromMemory(pBuffer, size, loadInfo));
}
...
cgl::PD3D11Texture2DBlank cgl::CD3D11Texture2DBlank::Create( D3D11_TEXTURE2D_DESC desc )
{
...
cgl::PD3D11BackBuffer cgl::CD3D11BackBuffer::Create()
{
	return create<CD3D11BackBuffer>(new CD3D11BackBuffer());
}

void cgl::CD3D11BackBuffer::onReset()
{
	SAFE_RELEASE((*ptr()));
}
