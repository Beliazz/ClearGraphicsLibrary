...
namespace cgl
{
	namespace core
	{
		...
		protected:
			CD3D11TextureFromMemory(void* pBuffer, size_t size, D3DX11_IMAGE_LOAD_INFO loadInf);
			HRESULT onRestore();

		public:
			static PD3D11TextureFromMemory Create(void* pBuffer, size_t size, D3DX11_IMAGE_LOAD_INFO loadInfo);
		};
		...
		public:
			static PD3D11Texture2DBlank Create(D3D11_TEXTURE2D_DESC desc);
		};
		...
		{
		private:
			PDXGISwapChain m_pSwapChain;

		protected:
			CD3D11BackBuffer(PDXGISwapChain pSwapChain);
			HRESULT onRestore();
			void onReset();
			void getDependencies(std::vector<PCGLObject>* pDependencies );

		public:
			static PD3D11BackBuffer Create(PDXGISwapChain pSwapChain);
		};
	}
}
